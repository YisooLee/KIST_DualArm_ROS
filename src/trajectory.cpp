...
void CTrajectory::reset_initial(double time0, VectorCXd init_pos, VectorCXd init_vel)
{
...
void CTrajectory::update_goal(VectorCXd goal_pos, VectorCXd goal_vel, double goal_time)
{
...
VectorCXd CTrajectory::position_cubicSpline()
{
	VectorCXd xd(_vector_size);	
	xd.setZero();
...
VectorCXd CTrajectory::velocity_cubicSpline()
{
	VectorCXd xdotd(_vector_size);
	xdotd.setZero();
...
void CTrajectory::check_vector_size(VectorCXd X)
{
